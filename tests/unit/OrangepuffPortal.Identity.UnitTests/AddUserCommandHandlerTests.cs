...
public class AddUserCommandHandlerTests
{
    ...
    public async Task Handle_creates_independent_user()
    {
        ...
        var handler = new AddUserCommandHandler(repo.Object, TestHelpers.CreatePasswordHasher(), publisher.Object, TestHelpers.CreateTransactionLogger().Object, NullLogger<AddUserCommandHandler>.Instance);

        var result = await handler.Handle(new AddUserCommand("newuser", "new@example.com", "New User", null, null, ActorUserId: 1), CancellationToken.None);
        ...
        publisher.Verify(p => p.Publish(It.Is<UserCreatedNotification>(n => n.ActorUserId == 1), It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task Handle_rejects_when_username_taken()
    {
        ...
        var handler = new AddUserCommandHandler(repo.Object, TestHelpers.CreatePasswordHasher(), Mock.Of<IPublisher>(), TestHelpers.CreateTransactionLogger().Object, NullLogger<AddUserCommandHandler>.Instance);

        var result = await handler.Handle(new AddUserCommand("taken", null, null, null, null, ActorUserId: 1), CancellationToken.None);
    ...
    public async Task Handle_rejects_when_template_user_is_not_usable()
    {
        ...
        var result = await handler.Handle(new AddUserCommand("newuser", null, null, 99, null, ActorUserId: 1), CancellationToken.None);
