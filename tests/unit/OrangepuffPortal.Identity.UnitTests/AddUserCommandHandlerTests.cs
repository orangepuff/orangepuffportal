...
public class AddUserCommandHandlerTests
{
    ...
    public async Task Handle_creates_independent_user()
    {
        ...
        var handler = new AddUserCommandHandler(repo.Object, TestHelpers.CreatePasswordHasher(), TestHelpers.CreateTransactionLogger().Object, NullLogger<AddUserCommandHandler>.Instance);

        var result = await handler.Handle(new AddUserCommand("newuser", "new@example.com", "New User", null, null), CancellationToken.None);
    ...
    public async Task Handle_rejects_when_username_taken()
    {
        ...
        var result = await handler.Handle(new AddUserCommand("taken", null, null, null, null), CancellationToken.None);
    ...
    public async Task Handle_rejects_when_template_user_is_not_usable()
    {
        ...
        var result = await handler.Handle(new AddUserCommand("newuser", null, null, 99, null), CancellationToken.None);
